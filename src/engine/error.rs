...
/// Regex patterns for parsing compiler output.
/// Each pattern extracts: file, line, col, code, message, suggestion.
struct ErrorRegex {
    pattern: Lazy<Regex>,
    kind: ErrorKind,
}
    ...
    pub fn parse(output: &str) -> Vec<CompileError> {
        ...
        while i < lines.len() {
            ...
            if let Some(cap) = parse_error_header(line) {
                let (code, message, is_warning) = cap;
                ...
                let mut col = 0u32;
                if i + 1 < lines.len() {
                    if let Some(loc) = parse_location(lines[i + 1]) {
                        file = loc.0;
                        line_no = loc.1;
                        col = loc.2;
                    }
                ...
                while j < lines.len() && j < i + 20 {
                    ...
                    if clean.starts_with("help:") || clean.starts_with("note:") {
                        suggestion = Some(clean.trim_start_matches("help:").trim_start_matches("note:").trim().to_string());
                    }
...
fn parse_location(line: &str) -> Option<(String, u32, u32)> {
    ...
    if let Some(rest) = trimmed.strip_prefix(prefix) {
        ...
        if parts.len() >= 3 {
            if let (Ok(line_num), Ok(col_num)) = (parts[parts.len()-2].parse::<u32>(), parts[parts.len()-1].parse::<u32>()) {
...
fn classify_error(code: &str, message: &str) -> ErrorKind {
    match code {
        c if c.starts_with("E0308") || message.contains("mismatched types") => ErrorKind::TypeMismatch,
        c if c.starts_with("E0425") || c.starts_with("E0432") || c.starts_with("E0433") => ErrorKind::UnresolvedSymbol,
        ...
        c if c.starts_with("E0107") || c.starts_with("E0282") || c.starts_with("E0283") => ErrorKind::GenericError,
        _ => {
            if message.contains("cannot find") { ErrorKind::UnresolvedSymbol }
            else if message.contains("expected") && message.contains("found") { ErrorKind::TypeMismatch }
            else if message.contains("import") { ErrorKind::MissingImport }
            else if message.contains("borrow") { ErrorKind::BorrowError }
