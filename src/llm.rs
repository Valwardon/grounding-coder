...
impl LlmClient {
    ...
    pub async fn translate(&self, prompt: &str) -> Result<StructuredIntent, String> {
        let system = concat!(
            ...
            "- define: array of null or { name: string, kind: string, references: [string], signature?: string, cases?: [{input: string, expected: string}], fields?: [{name: string, type: string}], methods?: [{name: string, self: \"none\"|\"ref\"|\"mut\", params: [\"n: Type\"], ret?: string, op: \"new\"|\"add_assign\"|\"get\"|\"call\"|\"knew\"|\"kcall\", field?: string, amount?: string}], title?: string, sections?: [{heading: string, body: string}], footer?: string } — NO code; cases/sections are literal values only; kind \"struct\" with fields+methods synthesizes behavior (Rust ops new/add_assign/get/call; Kotlin .kt targets use knew/kcall over kotlin.random.Random)\n",
